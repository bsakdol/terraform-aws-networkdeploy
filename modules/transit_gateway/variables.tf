...
variable "route_tables" {
  ...
  description = <<-EOT
    A map of boolean flags to determine which Transit Gateway route tables
    should be provisioned. VPCs tagged with `Environment = <route_table_name>`
    will be automatically attached to the corresponding route table.
  EOT
}
...
variable "transit_gateway_route_table_tags" {
  ...
  default     = {}
  description = <<-EOT
    A map of tags to assign to the TGW route table resource. Resource specific
